...
Experience = namedtuple('Experience', 'state0, action, reward, terminal1')
...
def sample_batch_indexes(low, high, size):
    ...
    if high - low >= size:
        ...
        # 我们有足够的数据。 绘制时无需替换，即每个索引在批处理中都是唯一的。 我们不能在这里使用`np.random.choice`，
        # 因为随着内存的增加它效率极低。 有关讨论，请参见https://github.com/numpy/numpy/issues/2764。
        # `random.sample`做同样的事情（绘图而无需替换），并且速度更快。
        try:
            r = xrange(low, high)
        except NameError:
            r = range(low, high)
    ...
    def sample(self, batch_size, batch_idxs=None):
        ...
        for idx in batch_idxs:
            ...
            for offset in range(0, self.window_length - 1):
                ...
                # t5 =time.time()
                # if t5-t1>0.00001:
                #     print(t5-t4,t4-t2,t2-t1,'    ',self.observations.length)
            while len(state0) < self.window_length:
                state0.append(zeroed_observation(state0[0]))
            state0.reverse()
            state0.append(self.observations[idx])
            action = self.actions[idx - 1]
            reward = self.rewards[idx - 1]
            terminal1 = self.terminals[idx - 1]

            assert len(state0) == self.window_length + 1
            experiences.append(Experience(state0=state0, action=action, reward=reward, terminal1=terminal1))
