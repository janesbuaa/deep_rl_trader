...
class AbstractDQNAgent(Agent):
    ...
    def __init__(self, nb_actions, memory, gamma=.99, batch_size=32, nb_steps_warmup=1000,
        ...
        if target_model_update <= 0:
            ...
            target_model_update = int(1 / target_model_update)
...
class DQNAgent(AbstractDQNAgent):
    ...
    def compile(self, optimizer, metrics=[]):
        ...
        updates = get_soft_target_model_updates(self.target_model, self.model, 1. / self.target_model_update)
    ...
    def backward(self, reward, terminal):
        ...
        if not self.training:
            ...
            # 我们在这里完成。 无需更新经验记忆，因为我们仅使用工作记忆来获取最新观测者的状态。
            return metrics
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            # 准备和验证参数。*********************************************************************************************
            batch = self.process_state_batch(state0_batch)
            state1_batch = batch[:,1:,:]
            state0_batch = batch[:,:-1,:]
            ...
            if self.enable_double_dqn:
                ...
                # 根据“使用双Q学习进行深度强化学习”（van Hasselt等人，2015）一文，在Double DQN中，
                # 现实网络可以预测动作，而目标网络则可以用来估计Q值。
                ...
                # 现在，使用目标网络估算Q值，但选择在线模型的Q值最高的值（如上计算）。
                ...
                # 计算给定state1的q_values，并提取批次中每个样本的最大值。 由于Mnih（2015）中概述的原因，
                # 我们对target_model而不是模型进行了这种预测。 简而言之：它使算法更稳定。
                target_q_values = self.target_model.predict_on_batch(state1_batch)
                assert target_q_values.shape == (self.batch_size, self.nb_actions)
            ...
            # 计算r_t + gamma * max_a Q（s_t + 1，a）并相应地更新目标目标，但仅针对受影响的输出单位（由action_batch给出）。
            ...
            for idx, (target, mask, R, action) in enumerate(zip(targets, masks, Rs, action_batch)):
                target[action] = R  # update action with estimated accumulated reward   用估计的累积奖励更新动作
                dummy_targets[idx] = R
                mask[action] = 1.  # enable loss for this specific action               为该动作启用损失
            ...
            # 最后，对整个批次执行一次更新。 我们使用虚拟目标，因为实际损失是在需要更复杂输入的Lambda层中计算的。
            # 但是，了解实际目标以正确计算指标仍然很有用。
            ins = [state0_batch] if type(self.model.input) is not list else state0_batch
            metrics = self.trainable_model.train_on_batch(ins + [targets, masks], [dummy_targets, targets])
            metrics = [metric for idx, metric in enumerate(metrics) if idx not in (1, 2)]
            # throw away individual losses      丢掉单独的损失
            metrics += self.policy.metrics
            if self.processor is not None:
                metrics += self.processor.metrics
            # t5 = time.time()
            # print(t5-t4,t4-t33,t33-t3,t3-t2,t2-t1)
            # 0.359375 0.0 0.4374978542327881 0.3124971389770508 2.843755006790161
